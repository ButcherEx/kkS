...
bool WINAPI Get_Module_By_Ret_Addr(PBYTE Ret_Addr, CHAR* Module_Name, PBYTE & Module_Addr)
{
		...
		::Module32First(hSnapshot, &M))
	{
		do
		{
			if (uint32_t(Ret_Addr - M.modBaseAddr) < M.modBaseSize)
			{
...
void CreateExceptionDesc(PEXCEPTION_POINTERS pException, FILE* fp, uint32_t dwLastError)
{
...
#pragma warning(default: 4200)
		...
		Ret_Addr_I++, Ebp = Ebp->Ebp)
	{
		...
		if (Get_Module_By_Ret_Addr(Ebp->Ret_Addr, Module_Name, Module_Addr_1))
		{
			...
			else if (!IsBadReadPtr(Ebp, sizeof(PSTACK) + 5 * sizeof(uint32_t)))
			{
...
void Get_Exception_Info(PEXCEPTION_POINTERS pException, FILE* fp, uint32_t dwLastError)
{
...
bool CreateBigInfoFile(PEXCEPTION_POINTERS pException, CHAR* szBigFile, uint32_t dwLastError)
{
