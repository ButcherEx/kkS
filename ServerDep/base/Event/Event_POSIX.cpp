...
EventImpl::EventImpl(bool autoReset): _auto(autoReset), _state(false)
{
	...
	if (pthread_mutex_init(&_mutex, NULL))
		BOOST_THROW_EXCEPTION(std::exception("cannot create event (mutex)"));
	if (pthread_cond_init(&_cond, NULL))
		BOOST_THROW_EXCEPTION(std::exception("cannot create event (condition)"));
}
...
void EventImpl::waitImpl()
{
	if (pthread_mutex_lock(&_mutex))
		BOOST_THROW_EXCEPTION(std::exception("wait for event failed (lock)")); 
	while (!_state) 
	{
		if (pthread_cond_wait(&_cond, &_mutex))
		{
			pthread_mutex_unlock(&_mutex);
			BOOST_THROW_EXCEPTION(std::exception("wait for event failed"));
		}
	}
	if (_auto)
		_state = false;
...
bool EventImpl::waitImpl(int32_t milliseconds)
{
	int rc = 0;
	...
	while (!_state) 
	{
		if ((rc = pthread_cond_timedwait(&_cond, &_mutex, &abstime)))
		{
			...
			pthread_mutex_unlock(&_mutex);
			BOOST_THROW_EXCEPTION(std::exception("cannot wait for event"));
		}
	}
