...
protected:
	...
	void setImpl();
	void waitImpl();
	bool waitImpl(int32_t milliseconds);
	void resetImpl();
...
inline void EventImpl::setImpl()
{
	if (pthread_mutex_lock(&_mutex))	
		BOOST_THROW_EXCEPTION(std::exception("cannot signal event (lock)"));
	_state = true;
	if (pthread_cond_broadcast(&_cond))
	{
		pthread_mutex_unlock(&_mutex);
		BOOST_THROW_EXCEPTION(std::exception("cannot signal event"));
	}
	pthread_mutex_unlock(&_mutex);
}
...
{
	if (pthread_mutex_lock(&_mutex))	
		BOOST_THROW_EXCEPTION(std::exception("cannot reset event"));
	_state = false;
	pthread_mutex_unlock(&_mutex);
}
